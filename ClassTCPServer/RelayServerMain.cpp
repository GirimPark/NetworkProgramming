...
int main()
{
	...
	netfish::TCPRelayServer::GetInstance()->Start();

	while(true)
	{
		netfish::TCPRelayServer::GetInstance()->NetUpdate();
		netfish::TCPRelayServer::GetInstance()->Process();
	}

	netfish::TCPRelayServer::GetInstance()->Stop();
	return 0;
}